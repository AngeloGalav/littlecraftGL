...
void key_callback(GLFWwindow* window, int key, int scancode, int action,
    ...
    // action buttons
    if (key == GLFW_KEY_Q && action == GLFW_PRESS) main_world.removeBlock();
    if (key == GLFW_KEY_E && action == GLFW_PRESS) main_world.addBlock();
...
/** Cambia la visuale in base alla posizione del mouse, senza che si faccia
 * click.
 *
 */
...
void rotateCameraKeyboard(int x, int y) {
    ...
    mainCamera.ViewSetup.target =
        mainCamera.ViewSetup.position + mainCamera.ViewSetup.direction;
}

// funzione che ruota la telecamera durante il gioco
void rotateCamera(int xpos, int ypos) {
    float alfa = 0.05;  // serve ridimensionare l'offset tra due posizioni
                        // successive del mosue
    ypos = height - ypos;
    if (firstMouse) {
        lastX = xpos;
        lastY = ypos;
