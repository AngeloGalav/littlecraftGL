...
void NewCube::init() {
    ...
    for (auto& pos : positions) {
        ...
        if (7 < j && j < 12)
            vertices.emplace_back(worldPos, colors[i], tex_coords_top[i]);
        else if (11 < j && j < 16)
            vertices.emplace_back(worldPos, colors[i], tex_coords_bot[i]);
        else
...
std::vector<GLuint>& NewCube::getIndices() { return indices; }
...
// get only a specific face
std::pair<int, int> NewCube::getFaceVertexRange(glm::ivec3 direction) {
    if (direction == glm::ivec3(1, 0, 0)) return {0, 4};     // FRONT
    if (direction == glm::ivec3(-1, 0, 0)) return {4, 8};    // BACK
    if (direction == glm::ivec3(0, 0, 1)) return {8, 12};    // TOP
    if (direction == glm::ivec3(0, 0, -1)) return {12, 16};  // BOTTOM
    if (direction == glm::ivec3(0, -1, 0)) return {16, 20};  // RIGHT
    if (direction == glm::ivec3(0, 1, 0)) return {20, 24};   // LEFT
    return {0, 0};  // Invalid face
}